...
class Rooftop:
    ...
    def dataframe(self): 
        self.df = pd.DataFrame(list(zip(self.street,self.city,self.state,self.zipcode,self.rooms,
                                   self.prices,self.rents,self.rate,self.gyield,self.rating,self.links)), 
               columns =['Street', 'City','State','Zip','Rooms','Price','Current Rents',
